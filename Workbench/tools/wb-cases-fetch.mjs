#!/usr/bin/env node
/* ============================================================
   wb-cases-fetch.mjs —— WB案例资讯 快照生成器
   ------------------------------------------------------------
   用途：把「人工/助手采集到的公众号 WorkBuddy 使用案例」写成
         src/js/08-ai-news-data.js，供构建期烘焙进单文件页面。

   为什么不在页面里实时抓：微信生态（搜狗微信搜索 / mp.weixin.qq.com）
   有反爬与跨域限制，浏览器直连必失败。与技能库模块同一套思路：
   页面只读快照，本脚本负责重新采集 + 重建。

   用法：
     node tools/wb-cases-fetch.mjs            # 用内置候选池重写数据文件
     node tools/wb-cases-fetch.mjs --dry      # 只打印统计，不写文件

   数据来源约束（用户 2026-09-12 明确）：
     · 总量 15 条/天：5 条腾讯社区 + 10 条公众号
     · 只收「小而明确可落地」的具体场景，泛泛而谈的感悟文不收
     · 同一种场景只收一条（按 scene 去重），优先保留公众号来源
     · 每条必须标注来源渠道（公众号名称 / 社区名）
   ============================================================ */
...
const OUT = path.join(ROOT, 'src', 'js', '08-ai-news-data.js');

const Q = 5;    /* 腾讯社区配额 */
const W = 10;   /* 公众号配额 */

/* ---- 候选池 ----
   fetch 语义：本脚本当前从 CANDIDATES 生成快照。
   后续如需自动采集，在 collectAuto() 里接入检索结果再合并进 CANDIDATES 即可。 */
const CANDIDATES = [
  /* ================= 腾讯社区（最多取 5，场景不重复） ================= */
  {
    sid: 'cc-pdf-contract', scene: '文档处理', srcType: 'community',
    source: '腾讯云开发者社区',
  ...
  {
    sid: 'cc-excel-merge', scene: '表格合并', srcType: 'community',
    source: '腾讯云开发者社区',
  ...
  {
    sid: 'cc-doc-renamer', scene: '文件批量重命名', srcType: 'community',
    source: '腾讯云开发者社区',
  ...
  {
    sid: 'cc-web-scrape', scene: '浏览器抓数据', srcType: 'community',
    source: '腾讯云开发者社区',
  ...
  {
    sid: 'cc-ppt-quick', scene: '做 PPT', srcType: 'community',
    source: '腾讯云开发者社区',
  ...
  /* ================= 公众号（取 10 条，场景不重复） ================= */
  {
    sid: 'gz-pipeline-6skill', scene: '公众号流水线', srcType: 'wechat',
    source: '公众号「老万AI日记」',
  ...
  {
    sid: 'gz-gzh-ops', scene: '公众号全流程托管', srcType: 'wechat',
    source: '公众号「启明AI电商学堂」',
  ...
  {
    sid: 'gz-notes-to-article', scene: '笔记转公众号长文', srcType: 'wechat',
  ...
  {
    sid: 'gz-workreport-dump', scene: '把碎片整理成报告提纲', srcType: 'wechat',
    source: '公众号「WorkBuddy 实战库」',
  ...
  {
    sid: 'gz-factpack', scene: '一套素材做多平台内容', srcType: 'wechat',
    source: '公众号「hunkcai」',
  ...
  {
    sid: 'gz-gzh-all-in-one', scene: '写文到排版一句话搞定', srcType: 'wechat',
    source: '公众号「点晴永久免费OA」',
  ...
  {
    sid: 'gz-wechat-api-push', scene: '程序化推送到草稿箱', srcType: 'wechat',
  ...
  {
    sid: 'gz-wechat-html-format', scene: '公众号 HTML 排版适配', srcType: 'wechat',
    source: '公众号「开发者的AI学习笔记」',
  ...
  {
    sid: 'gz-competitor-research', scene: '选题调研', srcType: 'wechat',
    source: '公众号「开发者的AI学习笔记」',
  ...
  {
    sid: 'gz-4pitfalls', scene: '避坑：AI 运营公众号的坑', srcType: 'wechat',
    source: '公众号「中场变量」',
...
/* ---- 去重规则（用户 2026-09-12 定）：同种场景只收一条，优先公众号 ---- */
function pickScenes(list) {
  const byScene = new Map();
  list.forEach((it) => {
    const cur = byScene.get(it.scene);
    if (!cur) { byScene.set(it.scene, it); return; }
    /* 同一场景冲突时：公众号优先，其次保留先出现的（人工已排好序） */
    if (cur.srcType !== 'wechat' && it.srcType === 'wechat') byScene.set(it.scene, it);
  });
  return [...byScene.values()];
}
const deduped = pickScenes(CANDIDATES);

const community = deduped.filter((c) => c.srcType === 'community').slice(0, Q);
const wechat = deduped.filter((c) => c.srcType === 'wechat').slice(0, W);
const final = [...wechat, ...community];

/* ---- 写文件 ---- */
const version = Number(
  (fs.existsSync(OUT)
    ? (fs.readFileSync(OUT, 'utf8').match(/ARTICLES_SEED_VERSION\s*=\s*(\d+)/) || [])[1]
    : 0) || 0
) + 1;

function jsStr(s) { return JSON.stringify(String(s)); }
function arr(a) { return '[' + a.map(jsStr).join(', ') + ']'; }

const lines = [];
lines.push('/* ============================================================');
lines.push('   08-ai-news-data.js —— WB案例资讯（快照，构建期烘焙）');
lines.push('   !!! 本文件由 tools/wb-cases-fetch.mjs 生成，请勿手改 !!!');
lines.push('   字段：sid / scene（场景，用于去重）/ srcType（wechat|community）');
lines.push('        / source（来源渠道）/ title / skills / howto（可落地做法）/ url');
lines.push('   收录标准：小而明确可落地，同种场景只收一条，优先公众号来源');
lines.push('   ============================================================ */');
lines.push('var ARTICLES_SEED_VERSION = ' + version + ';');
const today = new Date();
const pad = (x) => String(x).padStart(2, '0');
const dateStr = today.getFullYear() + '-' + pad(today.getMonth() + 1) + '-' + pad(today.getDate());
lines.push('var ARTICLES_FETCHED_AT = ' + jsStr(dateStr) + ';');
lines.push('');
lines.push('var ARTICLES_SEED = [');
final.forEach((c) => {
  lines.push('  {');
  lines.push('    sid: ' + jsStr(c.sid) + ',');
  lines.push('    scene: ' + jsStr(c.scene) + ',');
  lines.push('    srcType: ' + jsStr(c.srcType) + ',');
  lines.push('    source: ' + jsStr(c.source) + ',');
  lines.push('    title: ' + jsStr(c.title) + ',');
  lines.push('    skills: ' + arr(c.skills || []) + ',');
  lines.push('    howto: ' + jsStr(c.howto) + ',');
  lines.push('    url: ' + jsStr(c.url));
  lines.push('  }' + (final[final.length - 1] === c ? '' : ','));
});
lines.push('];');
lines.push('');

console.log('[wb-cases] 候选 ' + CANDIDATES.length +
  ' → 场景去重 ' + deduped.length +
  ' → 公众号 ' + wechat.length + ' + 社区 ' + community.length +
  ' = ' + final.length);
console.log('[wb-cases] 场景：' + final.map((c) => c.scene).join(' / '));

if (process.argv.includes('--dry')) {
  console.log('[wb-cases] --dry：未写文件');
} else {
  fs.writeFileSync(OUT, lines.join('\n'), 'utf8');
  console.log('[wb-cases] 已写入 ' + path.relative(ROOT, OUT) + '（version=' + version + '）');
}
